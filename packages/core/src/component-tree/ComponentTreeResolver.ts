...
function dynamicChildList(
  value: JsonValue,
): { path: string; componentId: string } | undefined {
  if (value === null || Array.isArray(value) || typeof value !== "object")
    return undefined;
  const object = value as JsonObject;
  return typeof object.path === "string" &&
    typeof object.componentId === "string"
  ...
  function* visit(
    ...
    if (offset === metadata.path.length) {
      ...
      if (leaf?.kind === "property")
        yield { value: current, location, property: leaf.name };
      return;
    }
    ...
    if (segment.kind === "property") {
      if (
        ...
        yield* visit(current[segment.name]!, offset + 1, [
          ...location,
    ...
    for (let index = 0; index < current.length; index += 1) {
      yield* visit(current[index]!, offset + 1, [
        ...location,
        { kind: "arrayIndex", index },
      ]);
    }
  }
  yield* visit(value, 0, []);
...
export class ComponentTreeResolver {
  ...
  resolve(
    ...
    return this.resolveFrom(surface, "root", budget);
  }
  ...
  resolveFrom(
    ...
    if (!this.catalogs.has(surface.catalogId)) {
      const missing = this.catalogs.getComponentStructureLocations(
        surface.catalogId,
        componentId,
      );
      if (!missing.ok)
    ...
    const resolveNode = (
      ...
      if (!counted) {
        const countError = budget.consumeTreeNode(component.id);
        if (countError !== undefined) return countError;
      }
      const metadata = this.catalogs.getComponentStructureLocations(
        surface.catalogId,
        component.component,
      );
      ...
      const resolveTarget = (
        ...
        const issueLocation = {
          ...
          propertyPath: pointer(location),
        };
        const countError = budget.consumeTreeNode(targetId);
        if (countError !== undefined) return countError;
        ...
        return resolveNode(target, ancestryPath, depth + 1, true);
      };
        ...
        for (const runtime of runtimeValues(component, structural)) {
          ...
          if (Array.isArray(value)) {
            ...
            for (const entry of value) {
              ...
              targetIds.push(entry);
              const node = resolveTarget(property, location, entry);
              if (node !== undefined && "code" in node) return node;
    ...
    const root = resolveNode(startingComponent, [], 1);
