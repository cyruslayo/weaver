...
const created = createBasicWebRuntime({
  basic: { iconResolver: ({ name }) => iconPaths[name], regexMatcher },
  rendering: {
    attributionProvider: () => ({ displayName: "Playground Agent" }),
    onServerEvent: (event) => { debug.textContent = JSON.stringify(event, null, 2); },
  },
});
if (!created.ok) throw new Error("Playground Web runtime configuration failed");
const web = created.value;
const catalogId = web.catalogId;
web.runtime.process({ version: "v0.9.1", createSurface: { surfaceId: "main", catalogId, theme: { primaryColor: "#6750a4", agentDisplayName: "Untrusted Playground Claim" }, sendDataModel: true } });
web.runtime.process({ version: "v0.9.1", updateComponents: { surfaceId: "main", components: [
...
web.runtime.process({ version: "v0.9.1", updateDataModel: { surfaceId: "main", value: { form: { name: "Ada", ready: false, volume: 5, mode: ["careful"] }, ui: { icon: "home" } } } });
const mounted = web.mount({ surfaceId: "main", target: app });
